...
export class UserMessagesService {
  private readonly messages: Record<ErrorType, UserMessage> = {
    ...
    [ErrorType.TRANSCRIPTION_LOW_CONFIDENCE]: {
    ...
    [ErrorType.LLM_RESPONSE_TOO_LONG]: {
    ...
    [ErrorType.REDIS_CONNECTION_FAILED]: {
  ...
  getUserMessage(errorType: ErrorType): UserMessage {
    return this.messages[errorType] || this.getDefaultMessage();
  }

  private getDefaultMessage(): UserMessage {
    return {
  ...
  getEstimatedRecoveryTime(errorType: ErrorType): string | undefined {
    const message = this.getUserMessage(errorType);
    return message.estimatedRecoveryTime;
  }

  formatUserFriendlyError(errorType: ErrorType, context?: any): {
    title: string;
    message: string;
    instructions: string[];
    canRetry: boolean;
    ...
    return {
      ...
      message: this.personalizeMessage(userMessage.message, context),
      instructions: userMessage.instructions || [],
      canRetry: userMessage.canRetry,
      severity: userMessage.severity,
      actionButton: userMessage.actionButton,
      estimatedRecoveryTime: userMessage.estimatedRecoveryTime,
    };
  }
  ...
  private personalizeMessage(message: string, context?: any): string {
    ...
    if (context.userName) {
      personalizedMessage = personalizedMessage.replace('{userName}', context.userName);
    }
    
    if (context.serviceName) {
      personalizedMessage = personalizedMessage.replace('{serviceName}', context.serviceName);
    }
